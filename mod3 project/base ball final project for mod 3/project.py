...
def read_csv_as_nested_dict(filename, keyfield, separator, quote):
    ...
    with open(filename, newline='') as csvfile:
        ...
        for row in csvreader:
            ...
            table[rowid] = row
    return table
#print(read_csv_as_nested_dict('Batting_2016.csv', 'playerID', ',', '"'))
...
def top_player_ids(info, statistics, formula, numplayers):
    ...
    final_list = [temp_list[x] for x in range(0,numplayers)]
...
def lookup_player_names(info, top_ids_and_stats):
    ...
    info_dict = read_csv_as_nested_dict(info['masterfile'], info['playerid'], \
                                        info['seperator'], info['quote'])
    for idx in top_ids_and_stats:
        appended_string = format(idx[1], '.3f') + '--- {0}{1}'. \
          format(info_dict[idx[0]][info['firstname']],info_dict[idx[0]][info['lastname']])
        final_list.append(appended_string)
    return final_list
...
def compute_top_stats_year(info, formula, numplayers, year):
    ...
    year = filter_by_year(stats_list, year, info['yearid'])
    top_players = top_player_ids(info, year, formula, numplayers)
...
def aggregate_by_player_id(statistics, playerid, fields):
    ...
    for row in statistics:
        if row[playerid] not in final_dict:
            ...
            for field in fields:
                stats[field] = int(row[field])

            final_dict[row[playerid]] = dict(stats)
        else: 
            for field in fields:
...
def compute_top_stats_career(info, formula, numplayers):
    ...
    """
    stats = []
    aggregated_stats = aggregate_by_player_id(read_csv_as_list_dict \
                                              (info['battingfile'], info['separator'], info['quote']), \
                                              info['playerid'], \
                                              info['battingfields'])
    for val in aggregated_stats.values():
        stats.append(val)
